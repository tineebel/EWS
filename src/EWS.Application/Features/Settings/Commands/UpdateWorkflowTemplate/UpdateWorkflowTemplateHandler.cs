    ...
    : IRequestHandler<UpdateWorkflowTemplateCommand, Result<int>>
{
    ...
    {
        // 1. Load template with steps
        var template = await db.WorkflowTemplates
            .Include(t => t.Steps)
            .FirstOrDefaultAsync(t => t.TemplateId == req.TemplateId, ct);

        if (template is null)
            return Result<int>.Fail("WF_TEMPLATE_NOT_FOUND", $"WorkflowTemplate {req.TemplateId} not found.");
        ...
        // Soft-delete steps not in command
        foreach (var existingStep in template.Steps)
        {
            if (!commandStepIds.Contains(existingStep.StepId))
            {
        ...
        foreach (var stepDto in req.Steps)
        {
            ...
            if (stepDto.StepId.HasValue)
            {
                ...
                if (existingStep is not null)
                {
                    ...
                    existingStep.ApproverType = approverType;
        ...
        return Result<int>.Success(template.TemplateId);
    }
}
